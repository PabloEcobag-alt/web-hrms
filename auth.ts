  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: new URLSearchParams({
        client_id: process.env.AUTH_HRMS_CLIENT_ID!,
        client_secret: process.env.AUTH_HRMS_CLIENT_SECRET!,
...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  callbacks: {
      ...
      if (profile) {
        ...
        if (profile.permissions) {
          try {
            ...
            // Only store HRMS permissions to avoid 431 Request Header Fields Too Large
            // caused by the NextAuth session cookie exceeding browser/Node limits.
            token.permissions = { HRMS: allPerms?.HRMS || {} };
          } catch {
            token.permissions = {};
          }
        }
    ...
    async authorized({ auth, request }) {
      ...
      const isProxyPath = pathname.startsWith("/api/hrms") || pathname.startsWith("/hubs/");
      ...
      if (auth?.user) {
        ...
        const hasHrmsAccess = systems.includes("HRMS");
        ...
        } else if (pathname.startsWith("/recruitment-hiring")) {
          requiredModule = "Recruitment and Hiring";
        } else if (pathname.startsWith("/digital-201-file")) {
          requiredModule = "Employee Information";
        } else if (pathname.startsWith("/payroll-deduction")) {
          requiredModule = "Payroll & ESS";
        } else if (pathname.startsWith("/attendance-biometrics")) {
          requiredModule = "Attendance Biometrics";
        } else if (pathname.startsWith("/recruitment-analytics")) {
          requiredModule = "Recruitment and Hiring";
        } else if (pathname.startsWith("/user-management")) {
          // User Management — SuperAdmin/CEO only.
          // Non-super users get denied.
          if (isAccessDeniedPage) return true;
          return Response.redirect(new URL("/access-denied", request.nextUrl));
        }

        if (requiredModule) {
          const hrmsPerms = auth.permissions?.HRMS;
          const hasModuleAccess = hrmsPerms?.[requiredModule]?.canRead;
          if (!hasModuleAccess) {
            if (isAccessDeniedPage) return true;
            return Response.redirect(new URL("/access-denied", request.nextUrl));
          }

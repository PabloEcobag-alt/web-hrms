...
// Allow self-signed certificates for the HRMS API in development
...
async function proxyRequest(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  const url = new URL(targetPath, HRMS_API_URL);
  ...
  try {
    ...
    console.error(`HRMS API proxy error for ${request.method} ${url.toString()}:`, errMsg);
    return NextResponse.json(
      { error: `Failed to connect to HRMS API: ${errMsg}` },
      { status: 502 }
    );
  }
}
